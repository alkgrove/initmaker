...
#defmacro eic
	...
	/* Clock EIC with %toupper(ref_source)% (%frequency(ref_source)%) */
...
#iftrue ref_source == "osculp32k"
	eic_set_CTRLA(EIC_CTRLA_CKSEL); // EIC clocked from %toupper(ref_source) (32.768KHz)
#otherwise
	eic_clear_CTRLA(EIC_CTRLA_CKSEL); // EIC clocked from %toupper(ref_source)% (%frequency(ref_source)%)
#fi
#ifdefined nmi
...
#isr %pinname% EIC_%eicnumber%_IRQn EIC_%eicnumber%_Handler
#endfor
#endmacro
